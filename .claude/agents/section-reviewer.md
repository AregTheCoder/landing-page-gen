...
1. Read `brief.md`, `workflow.yaml`, `result.md`, and any earlier
...
2. Score the final asset with the asset rubric in `evaluation.md`. Compare
   side by side with the examples: framing, density, finish. Check the
   shared context (hero) for consistency.
3. Score the workflow: pattern fit for the slot and its source media, every
   ...
   (path given in your prompt), within the advisory cap.
...
- `clean` below 5 (any text, logo, watermark, UI) is an automatic rework.
