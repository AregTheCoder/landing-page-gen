...
| composition | For a composite (a slot with a `composition-<slot>.yaml` plan): does every block the worker picked (`blocks-<slot>.yaml`) belong in this section — its `because:` true, the bank block's *When* (`compose/assets/blocks.yaml`) true of the copy, the page and the picture — and is it drawn in its slot, saying what the pick gave it, nothing twice, no empty slot drawn? A block the section does not call for (a font-pairing tile beside a photo, a palette on a calculator card, a tool the page never names), or text out of place with the picture (an example that is not this picture's own, a line that would read the same beside any picture), is a 2; one block missing or mis-placed is a 3; a string the pick did not give is a 1. A `rendered_by: model` item (the hybrid exception — brush-mask, applied-mockup, face-box) is judged here too: painted, organic, matching its `reason`, carrying no text; a model item that drew a string or added UI beyond its item is a 1. `5` when the slot has no composition plan (not a composite). The hard rules (a block of the slot's category and shape, a true claim, copy strings) are `--check-blocks` and precheck's; this score is the context judgement they cannot make. |
...
| clean | No logos, watermarks or UI, and no text beyond the brief's `## Text in image` strings? Chrome drawn by `lp-compose` is clean only when it is the picked blocks' (Before, After, x2, 4K, a copy line — page strings, drawn by compose) and none duplicates a model-rendered string. (a failure here is a hard reject) |
...
| motion | Does what changes across the strip match the node's prompt and the family's **Motion:** line (kind, camera, pace), with no morphing, no added or lost elements, no animated typography (a timeline's typed strings excepted: exact `> chrome:` text, drawn by lp-compose)? (below 3 is a rework) |
